...
class HybridCaptioner(nn.Module):
    ...
    def __init__(self, name, device):
        ...
        if self.global_length > 0:
            self.global_embeds = nn.Parameter(
                torch.rand(1, self.global_length, out_dim))

        # construct network that produces cond seq
        if self.mode == "Img":
            if self.feature_type == "Head":
                self.positional_embedding = nn.Parameter(
                    torch.rand(1, self.cond_length, out_dim))
    ...
    def check_config(self):
        ...
        if self.mode not in ["Img", "Pair"]:
            raise ValueError(
                f"Unsupported mode {self.mode} in Hybrid Captioner. Choose between ['Img', 'Pair'].")

        if self.feature_extractor not in ["CLIP", "DINO"]:
            raise ValueError(
                f"Unsupported feature extractor: {self.feature_extractor} in Hybrid Captioner. Choose between ['CLIP', 'DINO'].")

        if self.feature_type not in ["Feat", "Head"]:
            raise ValueError(
                f"Unsupported feature type: {self.feature_type} in Hybrid Captioner. Choose between ['Feat', 'Head'].")

        if self.global_length > self.max_learnable_length:
            raise ValueError(
                f"Global length {self.global_length} is longer than max length {self.max_learnable_length}.")

        if self.cond_length > self.max_learnable_length:
            raise ValueError(
                f"Conditional length {self.cond_length} is longer than max length {self.max_learnable_length}.")
    ...
    def forward(self,
        ...
        if self.mode == 'Pair':
            if self.feature_type == "Head":
                if self.feature_extractor == "CLIP":
                    src_feat = src_feature.image_embeds
                    trg_feat = trg_feature.image_embeds
                elif self.feature_extractor == "DINO":
                    src_feat = src_feature.pooler_output
                    trg_feat = trg_feature.pooler_output
            elif self.feature_type == "Feat":
                src_feat = src_feature.last_hidden_state[:, 1:, :]
                trg_feat = trg_feature.last_hidden_state[:, 1:, :]
        ...
        # put up everything
        ids = self.tokenizer("", truncation=True, return_tensors="pt")[
            "input_ids"].to(self.device)

        fixed_token_length = ids.shape[-1]
        actual_learnable_length = self.global_length + self.cond_length
        padding_length = self.max_learnable_length + 2 - \
            fixed_token_length - actual_learnable_length

        dummy_ids = torch.randint(
            0, self.tokenizer.vocab_size, (1, actual_learnable_length), device=self.device)
        padding_ids = torch.zeros(
            1, padding_length, dtype=torch.int64, device=self.device)

        ids = torch.cat(
            [ids[:, :-1], dummy_ids, ids[:, -1:], padding_ids], dim=-1)
