...
class CustomStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    def _encode_prompt(
        ...
        if prompt_embeds is None:
            if token_embeds is None:
                ...
                hidden_states = self.text_encoder.text_model.embeddings(input_ids=text_input_ids.to(device), position_ids=None)
            else:
                hidden_states = token_embeds.to(device)
                attention_mask = None
                input_shape = token_embeds.shape[:-1]
    ...
    def check_inputs(
        ...
        if prompt is None:
            if prompt_embeds is None:
                if token_embeds is None:
                    raise ValueError(
                        "Provide either `prompt`, `prompt_embeds` or `token_embeds`. Cannot leave `prompt`, `prompt_embeds` and `token_embeds` undefined."
                        )
                else:
                    pass
            else:
                if token_embeds is None:
                    ...
                    raise ValueError(
                        f"Cannot forward both `prompt_embeds` and `token_embeds`. Please make sure to"
            ...
            if prompt_embeds is None:
                if token_embeds is None:
                    ...
                    raise ValueError(
                        f"Cannot forward both `prompt` and `token_embeds`. Please make sure to"
                        " only forward one of the two."
                        )
            else:
                if token_embeds is None:
                    ...
                    raise ValueError(
                        f"Cannot forward `prompt`, `prompt_embeds` and `token_embeds`. Please make sure to"
                        " only forward one of the three."
                        )
    ...
    def __call__(
        ...
        self.check_inputs(
            prompt, height, width, callback_steps, negative_prompt, token_embeds, prompt_embeds, negative_prompt_embeds
        )
